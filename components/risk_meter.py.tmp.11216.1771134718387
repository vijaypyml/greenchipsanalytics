...
def calculate_risk_sentiment(market_data_dict):
    ...
    Logic:
    - Risk-On: Stocks ↑, VIX ↓, Crypto ↑, Commodities ↑, DXY ↓
    - Risk-Off: Stocks ↓, VIX ↑, Crypto ↓, Commodities ↓, Gold ↑, DXY ↑
...
def render_risk_meter(market_data_dict):
    ...
    fig = go.Figure(go.Indicator(
        ...
        value=score,
        domain={'x': [0, 1], 'y': [0, 1]},
        title={'text': f"{emoji}", 'font': {'size': 20}},
        number={'font': {'size': 24}},
        gauge={
            'axis': {'range': [-100, 100], 'tickwidth': 1, 'tickcolor': "#6c757d"},
            'bar': {'color': color, 'thickness': 0.6},
            'bgcolor': "rgba(0,0,0,0)",
            'borderwidth': 1,
            'bordercolor': "#dee2e6",
            'steps': [
                {'range': [-100, -30], 'color': 'rgba(255, 71, 87, 0.15)'},
                {'range': [-30, 30], 'color': 'rgba(255, 165, 2, 0.15)'},
                {'range': [30, 100], 'color': 'rgba(0, 212, 138, 0.15)'}
            ],
            'threshold': {
    ...
    # Display the compact gauge
    st.plotly_chart(fig, use_container_width=True, config={'displayModeBar': False})

    # Display compact regime interpretation
    st.markdown(f"""
    <div style="text-align: center; padding: 8px; background: linear-gradient(135deg, rgba(248, 249, 250, 0.8) 0%, rgba(255, 255, 255, 0.9) 100%); border-radius: 8px;">
        <p style="color: {color}; margin: 0; font-size: 1.1rem; font-weight: 700;">{regime}</p>
